...
rewards:
  enabled: false            # 奖励钩子总开关，详见 [奖励钩子与外部接入](./battlepass)
i18n:
...
首次启动时，Flyway 会执行迁移，建立玩家 / 比赛 / 队伍 / 职业进度 / 传奇首获等核心表。`db/migration/` 下的 `V*__*.sql` 是当前生效的 schema。
...
| 数据库里 `flyway_schema_history` 表 | 有迁移记录 |
